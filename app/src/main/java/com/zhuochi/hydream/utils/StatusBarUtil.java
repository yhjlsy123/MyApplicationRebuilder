    ...
    public static void setColor(Activity activity, int color, int statusBarAlpha) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            try {
                ...
                field.setInt(activity.getWindow().getDecorView(), activity.getResources().getColor(color));  //改为透明
            } catch (Exception e) {
            }
        }
        boolean xm = setMIUIStatusBarLightMode(activity.getWindow(), true);
        if (!xm) {
