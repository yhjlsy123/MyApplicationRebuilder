...
//        if (changed) {
        absListViews.clear();
        getAllAbsListView(absListViews, (ViewGroup) target);
//        }
    ...
    private void getAllAbsListView(List<AbsListView> absListViews, ViewGroup parent) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (child instanceof AbsListView) {
                ...
                getAllAbsListView(absListViews, (ViewGroup) child);
            }
        }
    }

    private List<AbsListView> absListViews = new ArrayList<>();
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        LogUtils.e("dispatchTouchEvent", "canChild pull donw " + canChildScrollUp2(ev));
        ...
        switch (actionMasked) {
            case MotionEvent.ACTION_DOWN:
                canMoveDown = !canChildScrollUp2(ev);
                if (!canMoveDown)
                    return super.dispatchTouchEvent(ev);
...
//                    boolean canMoveDown = canChildScrollUp();
    ...
    private void moveSpinner(float diff) {
        ...
        if (currentTargetOffsetTop > START_POSITION && targetY <= START_POSITION && !isTouch) {
...
//        Toast.makeText(getContext(), state.toString(), Toast.LENGTH_SHORT).show();
     ...
     * @return
     */
    private boolean canChildScrollUp2(MotionEvent ev) {
        for (AbsListView view : absListViews) {
            Rect mRect = new Rect();
            view.getGlobalVisibleRect(mRect);
            if (mRect.contains((int) ev.getX(), (int) ev.getY())) {
                boolean canChildScrollUp = view.getChildCount() > 0
                        && (view.getFirstVisiblePosition() > 0 || view.getChildAt(0)
                        .getTop() < view.getPaddingTop());
